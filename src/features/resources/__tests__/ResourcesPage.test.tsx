 ...
 * prompt de Areas): a mesma capacidade/alocacao ja usada em "Por equipe",
 * so' agrupada por Area - sem formula divergente, sem dupla contagem.
 */
...
const capacity = [
  { profile_id: 'p1', full_name: 'Ana Ribeiro', team_id: 't1', team_name: 'Contabil', reference_month: month, capacity_hours: 100, allocated_hours: 120, allocation_pct: 120, project_count: 2, area_id: 'a1', area_name: 'Contabilidade Geral', business_unit_id: 'bu1', business_unit_name: 'Contabilidade Corporativa' },
  { profile_id: 'p2', full_name: 'Carlos Menezes', team_id: 't1', team_name: 'Contabil', reference_month: month, capacity_hours: 100, allocated_hours: 60, allocation_pct: 60, project_count: 1, area_id: 'a1', area_name: 'Contabilidade Geral', business_unit_id: 'bu1', business_unit_name: 'Contabilidade Corporativa' },
  { profile_id: 'p3', full_name: 'Helena Duarte', team_id: 't2', team_name: 'Fiscal', reference_month: month, capacity_hours: 100, allocated_hours: 50, allocation_pct: 50, project_count: 1, area_id: 'a2', area_name: 'Fiscal e Tributario', business_unit_id: 'bu2', business_unit_name: 'Fiscal Corporativo' },
];

const allocations = [
  { id: 'al1', project_id: 'pr1', profile_id: 'p1', team_id: 't1', role_label: 'Analista', period_start: `${month.slice(0, 7)}-01`, period_end: `${month.slice(0, 7)}-28`, allocated_hours: 80, allocation_pct: 80, profile: { full_name: 'Ana Ribeiro' }, project: { code: 'PRJ-1', name: 'Fechamento Anual' } },
  { id: 'al2', project_id: 'pr2', profile_id: 'p1', team_id: 't1', role_label: 'Analista', period_start: `${month.slice(0, 7)}-01`, period_end: `${month.slice(0, 7)}-28`, allocated_hours: 40, allocation_pct: 40, profile: { full_name: 'Ana Ribeiro' }, project: { code: 'PRJ-2', name: 'Migracao ECD' } },
];

const areas = [
  { id: 'a1', business_unit_id: 'bu1', code: 'CTG', name: 'Contabilidade Geral', is_active: true, manager: { id: 'p1', full_name: 'Ana Ribeiro' } },
  { id: 'a2', business_unit_id: 'bu2', code: 'FIS', name: 'Fiscal e Tributario', is_active: true, manager: { id: 'p3', full_name: 'Helena Duarte' } },
];
...
describe('Recursos & Capacidade - tab Por area', () => {
  it('a nova tab fica ao lado de Por equipe, na ordem esperada', async () => {
    render();
    const abas = await screen.findAllByRole('tab');
    const labels = abas.map((a) => a.textContent);
    expect(labels.indexOf('Por equipe') + 1).toBe(labels.indexOf('Por area'));
  });
  ...
  it('agrega por area somando capacidade e horas (nao a media dos percentuais)', async () => {
    ...
    const linha = await screen.findByText('Contabilidade Geral').then((el) => el.closest('tr')!);
    ...
    const outraLinha = screen.getByText('Fiscal e Tributario').closest('tr')!;
  ...
  it('filtra por gerencia', async () => {
    render();
    await userEvent.click(screen.getByRole('tab', { name: 'Por area' }));
    await screen.findByText('Contabilidade Geral');
    await userEvent.selectOptions(screen.getByLabelText(/filtrar por gerencia/i), 'Fiscal Corporativo');
    expect(screen.queryByText('Contabilidade Geral')).toBeNull();
    expect(screen.getByText('Fiscal e Tributario')).toBeInTheDocument();
  });
  ...
  it('filtra por status de capacidade (atencao/sobrecarga)', async () => {
    ...
    expect(screen.getByText('Contabilidade Geral')).toBeInTheDocument();
    expect(screen.queryByText('Fiscal e Tributario')).toBeNull();
  });
